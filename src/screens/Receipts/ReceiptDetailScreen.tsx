    ...
    try {
      ...
      if (!blePrinter.isSupported()) {
        Alert.alert(
          ...
          'Bluetooth printing is not available on this device. Would you like to share the receipt instead?',
          [
      ...
      if (!isEnabled) {
        const enableResult = await blePrinter.enableBluetooth();
        if (!enableResult) {
      ...
      if (blePrinter.isSupported()) {
        await blePrinter.printReceipt({
          ...
          time: new Date(order.createdAt).toLocaleTimeString(),
          table: table?.name || order.tableId.replace('table-', ''),
          items: order.items.map((i: any) => ({ name: i.name, description: (i as any).description, quantity: i.quantity, price: i.price })),
        ...
        Alert.alert('Success!', 'Receipt sent to Bluetooth printer successfully');
      } else {
        // Fallback: generate PDF and share
        await printDocument({
          type: 'receipt',
          title: 'Receipt',
          content: {
            ...
            date: new Date(order.createdAt).toLocaleDateString(),
            time: new Date(order.createdAt).toLocaleTimeString(),
            tableNumber: table?.name || order.tableId,
            customerName: order.payment?.customerName || 'Customer',
            items: order.items.map((i: any) => ({ name: i.name, description: (i as any).description, quantity: i.quantity, price: i.price, total: i.price * i.quantity })),
            subtotal,
            tax,
            serviceCharge,
            discount,
          ...
          <View style={styles.receiptInfo}>
            ...
            {order.payment?.customerName && (
              <View style={styles.infoRow}>
                ...
                <Text style={styles.infoValue}>{order.payment.customerName}</Text>
              </View>
            )}
          </View>
